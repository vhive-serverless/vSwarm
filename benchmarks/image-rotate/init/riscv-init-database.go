...
func main() {
	...
	err = tmpSession.Query(`
		CREATE KEYSPACE IF NOT EXISTS image_rotate_db
		WITH replication = {
	...
	cluster.Keyspace = "image_rotate_db"
	...
	for _, file := range files {
		if isImageFile(file.Name()) {
			...
			if err := uploadFile(session, imagePath); err != nil {
				log.Warnf("Failed to upload %s: %v", file.Name(), err)
				continue
			}
			log.Infof("Inserted image: %s", file.Name())
		}
	}
