...
func NewProfiler(executionTime float64, printInterval uint64, level int, nodes, outFile string, socket, cpu int) (*Profiler, error) {
	...
	profiler.outFile = outFile

	profiler.cmd = exec.Command("/usr/local/pmu-tools/toplev",
		"-v",                // verbose output
		"--no-desc",         // no event description
		"-x",                // csv format
		",",                 // csv comma separated
		"-l",                // specify measurement level
		strconv.Itoa(level), // level
		"-I",                // measurement intervals in ms
		strconv.FormatUint(profiler.interval, 10), // interval
		"-o", // Set output file
		profiler.outFile)
